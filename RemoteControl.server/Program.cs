...
{
    // ¬ј∆Ќќ: чтобы не логоффало всех пользователей подр€д, ограничь проверку только автологон-юзером
    const string AutoLogonUser = "YOUR_AUTOLOGON_USER"; // <-- помен€й
    ...
    // LOCAL: ждЄм физическую активность мыши N секунд
    ...
    while (DateTime.UtcNow < deadline)
    {
        ...
        if (Math.Abs(cur.X - start.X) > 5 || Math.Abs(cur.Y - start.Y) > 5)
            return; // мышь шевельнули Ч значит локальный вход руками, не логоффаем
    }

    // никого нет Ч выкидываем
    try
    {
